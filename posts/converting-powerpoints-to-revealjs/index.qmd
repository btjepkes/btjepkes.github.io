...
keywords:
  ...
  - "Quarto"
  - "PowerPoint"
description: "In this post, I run through how I convert my standard PowerPoints (in .pptx format) into Revealjs versions to create interactive slides. There are likely more proper ways of completing this workflow, but this is what has worked for me so far."
...
Since switching to Quarto, I have also started using Reveal.js for most of my presentations. However, there are certain situations where I need to use PowerPoint due to venue restrictions. I also have a bunch of past conference and project presentations that I would like to display interactively on my site. How can I do this? Well, I have devised a workflow that I am not sure is the best way, but it works for me until someone tells me otherwise. It is important to note that while this *converts* my PPT files, any transitions or animations are lost because of my initial step of saving to a PDF.
...
Create a Quarto (.qmd) document saved in the directory where you want to create your HTML output. Then, specify your YAML header as you see fit. I have removed the menu with `menu: false` but kept the arrow slide controls with `controls: true` in the below example.
...
Once your HTML document is created, it is simple to embed it as an iframe into your site. There are other ways to configure this when `embed-resources: false` is set in your YAML, but my slides are not too large so this technique has worked for me.
...
So there you have it, how I convert my PowerPoint slides to reveal.js using Quarto in R. If anyone has a simpler method for doing so, I would love to hear about it.
