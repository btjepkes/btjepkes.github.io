...
subtitle: From coding and stats, to making maps.
...
items <- readr::read_csv("./resources.csv") |> 
  dplyr::select(Title, Type, Description, Link) |> 
  dplyr::rename(name = Title,
                category = Type,
                'custom-field' = Description,
                href = Link)

items_list <- apply(items, 1, function(row) as.list(row))

items_list <- lapply(items_list, function(x) {
  x <- lapply(x, as.character)  # Ensure all values are character strings
  x
})
